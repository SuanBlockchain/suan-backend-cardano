...
def check_token_in_datum(oracle_datum: DatumOracle, token_policy_id: bytes, token_name: bytes) -> None:
...
def check_oracle_datum(reference_inputs: List[TxInInfo], oracle_policy_id: bytes, token_policy_id: bytes, token_name: bytes) -> None:
...
def validator(oracle_policy_id: bytes, datum: DatumSwap, redeemer: ListingAction, context: ScriptContext) -> None:
    ...
    if isinstance(redeemer, Buy):
        ...
        check_oracle_datum(tx_info.reference_inputs, oracle_policy_id, tokenA_policy_id, tokenA_name)
        ...
        tokenA_quantity_spend = own_utxo.value.get(datum.tokenA.policy_id, {b"": 0}).get(datum.tokenA.token_name, 0)
        ...
        for txo in outputs:
            if txo.address.payment_credential.credential_hash == datum.owner:
                ...
                if txo.value.get(tokenB_policy_id, {b"": 0}).get(tokenB_name, 0) >= tokenB_quantity:
                    res = True
