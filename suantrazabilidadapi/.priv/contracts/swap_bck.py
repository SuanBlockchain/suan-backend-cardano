...
def validator(nft_policy_id: bytes, nft_token_name: bytes, datum: DatumSwap, redeemer: ListingAction, context: ScriptContext) -> None:
    ...
    # It is recommended to explicitly check all options with isinstance for user input
        if isinstance(redeemer, Buy):
            ...
            assert own_utxo.value.get(nft_policy_id, {b"": 0}).get(nft_token_name, 0) == 1, "nft token not found in utxo"
            
            outputs = tx_info.outputs
            tokenB_policy_id = datum.tokenB.policy_id
            tokenB_name = datum.tokenB.token_name

            tokenA_quantity_input = all_tokens_unlocked_from_address(tx_info.inputs, own_addr, datum.tokenA)
