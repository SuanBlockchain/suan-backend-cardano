...
def check_correct_amount(
    outputs: List[TxOut], datum: DatumProjectParams, payment: int
) -> None:
...
def check_oracle_datum(
    ...
    check_datum_price(
        oracle_datum, token_policy_id, token_name, token_balance, outputs, datum
    )
...
def validator(
    ...
    if isinstance(redeemer, Buy):
        tb = token_balance(
            tx_info.outputs, own_utxo, own_addr, token_policy_id, token_name
        )
