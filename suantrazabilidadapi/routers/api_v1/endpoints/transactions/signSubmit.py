...
async def sign_submit(signSubmit: pydantic_schemas.SignSubmit, mockSubmit: bool = False) -> dict:
    try:
        ########################
        """1. Get wallet info"""
        ########################
        ...
        if not final_response["success"]:
            raise ValueError( f"Wallet with id: {signSubmit.wallet_id} does not exist in DynamoDB")
        ...
        if signSubmit.scriptIds:
            for scriptId in signSubmit.scriptIds:
                ...
                if not final_response["success"]:
                    raise ValueError("Script does not exist in database")
