...
fastapi_configuration = config(section="configuration")
ENABLE_LOCAL_ENDPOINTS = fastapi_configuration["enable_local_endpoints"]
# Define a mapping from string values to boolean values
...
# Convert the config value to a boolean using the mapping
if ENABLE_LOCAL_ENDPOINTS in string_to_bool_mapping:
    ENABLE_LOCAL_ENDPOINTS = string_to_bool_mapping[ENABLE_LOCAL_ENDPOINTS]
else:
    # Handle cases where the value is not recognized
    ENABLE_LOCAL_ENDPOINTS = False  # Default to False or raise an error
...
async def get_forms_from_kobo() -> dict:
    ...
    for form in my_forms:
        ...
        if deployment_active and has_deployment and owner == km.SUANBLOCKCHAIN and km.desc_name_1 in name and km.desc_name_2 in name:
...
if ENABLE_LOCAL_ENDPOINTS:
    ...
    async def create_koboForms() -> dict:
        ...
        # Filter forms active, deployed and owner username = 'suanblockchain'
        filtered_forms = km.filter_form(my_forms)
    ...
    async def create_dataForms(db: Session = Depends(get_db)) -> dict:
        ...
        if forms == []:
            ...
            filtered_forms = km.filter_form(forms)
