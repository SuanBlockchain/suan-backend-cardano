            ...
            if walletInfo is None:
                ...
                for address in addresses:
                    ...
                    if address.lovelace <= min_val:
                        builder.add_output(TransactionOutput(Address.decode(address.address), Value(min_val, multi_asset), datum=datum))
                    else:
            ...
            if walletInfo is None:
                ...
                if send.addresses:
                    ...
                    if amount > 0:
                        # Calculate the minimum amount of lovelace that need to be transfered in the utxo  
                        min_val = min_lovelace(
                            chain_context, output=TransactionOutput(master_address, Value(0, mint_multiassets), datum=datum)
                        )
                        builder.add_output(TransactionOutput(master_address, Value(min_val, mint_multiassets), datum=datum))
