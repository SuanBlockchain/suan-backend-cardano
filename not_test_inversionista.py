...
def test_unlock_unlist(
    contract_info: dict, tokenName: str, unlockQ: int
) -> py.Transaction:
    context = contract_info["context"]
    administrador = contract_info["administrador"]
    spend_address = contract_info["spend_address"]
    ...
    # MultiAsset to trade
    multi_asset_unlist = build_multiAsset(parent_mint_policyID, tokenName, unlockQ)
    ...
    # Build Transaction Output to buyer
    min_val = min_value(context, administrador.address, multi_asset=multi_asset_unlist)
    tx_builder.add_output(
        py.TransactionOutput(
            administrador.address, py.Value(min_val, multi_asset_unlist)
        )
    )

    # tx_body = tx_builder.build(change_address=administrador.address)
    tx_signed = tx_builder.build_and_sign(
        [administrador.signing_key], change_address=propietario.address
    )
    return tx_signed


def test_burn(
    contract_info: dict, user: MockUser, tokenName: str, burnQ: int
) -> py.Transaction:
    context = contract_info["context"]
    mint_contract = contract_info["mint_contract"]
    parent_mint_policyID = contract_info["parent_mint_policyID"]
    ...
    tx_builder = py.TransactionBuilder(context)
    signatures = []
    signatures.append(py.VerificationKeyHash(bytes(user.address.payment_part)))
    signatures.append(py.VerificationKeyHash(bytes(administrador.address.payment_part)))
    # Build redeemer
    ...
    # Add input address to pay fees. This is the buyer address
    tx_builder.add_input_address(user.address)

    signed_tx = tx_builder.build_and_sign(
        [user.signing_key, administrador.signing_key],
    ...
    # if isinstance(context, MockChainContext):
    #     context.submit_tx(signed_tx)
    #     logging.info(f"Buyer: {user.balance()}")
    # else:
    #     result = context.evaluate_tx(signed_tx)
    #     context.submit_tx(signed_tx)
...
def create_oracle(
    contracts_info: dict, token_name: str, price: int, validity: int
) -> str:
    context = CardanoNetwork().get_chain_context()
    suanOracle = setup_user(context, walletName="suanOracleTest")

    token_policy_id = contracts_info["parent_mint_policyID"]

    # Build stakeholder address or pkh
    administrador = setup_user(context, walletName="administrador")
    bioC = setup_user(context, walletName="BioC")
    buffer = setup_user(context, walletName="buffer")
    comunidad = setup_user(context, walletName="comunidad")
    propietario = setup_user(context, walletName="propietario")
    spend_policy_id = contracts_info["spend_contract"].policy_id
    ...
    value_dict = {}
    distribution = {
        administrador.pkh.payload: 209,
        propietario.pkh.payload: 980,
        comunidad.pkh.payload: 352,
        bioC.pkh.payload: 456,
        buffer.pkh.payload: 589,
        bytes(spend_policy_id, encoding="utf-8"): 785,
    }
    periods = {
        b"p_1": {
            b"net_vol": 0,
...
if __name__ == "__main__":
    ...
    if contracts_info["utxo_to_spend"]:
        tx_id = create_oracle(contracts_info, tokenName, price, validity=1878695)
    ...
    ############################
    propietario = setup_user(contracts_info["context"], walletName="propietario")
    tx_signed = test_burn(contracts_info, propietario, tokenName, buyQ - 1)
    logging.info(
        f"burned project tokens with tx_id: {tx_signed.transaction_body.hash().hex()}"
    )
    ...
    # ############################
    # # Burn the purchased token (Propietario)
    # ############################
    inversionista = setup_user(contracts_info["context"], walletName="inversionista")
    tx_signed = test_burn(contracts_info, propietario, tokenName, qTokenA)
