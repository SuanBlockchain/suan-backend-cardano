...
from opshin.builder import PlutusContract, PlutusData, build
from opshin.prelude import Address as OpAddress
...
def get_nft_utxo(context: Any, address: Address, nft: MultiAsset) -> UTxO:
    ...
    nft_utxo = next(utxo for utxo in utxos if utxo.output.amount.multi_asset == nft)
    return nft_utxo
...
def get_signing_info(name):
    skey_path = str(
        f"/home/cardanodatos/suan/suan-trazabilidad/suantrazabilidadapi/.priv/wallets/keys/{name}.skey"
    )
...
def main(script: str, option: str = "send"):
    # Load chain context
    ...
    if r["data"].get("data", None) is not None:
        ...
        if walletInfo is None:
            ...
            payment_pkh = binascii.hexlify(
                payment_verification_key.hash().payload
            ).decode("utf-8")
            stake_pkh = binascii.hexlify(
                staking_verification_key.hash().payload
            ).decode("utf-8")
    ...
    cbor_path = Constants.PROJECT_ROOT.joinpath(Constants.CONTRACTS_DIR).joinpath(
        "build/script.cbor"
    )
    mainnet_path = Constants.PROJECT_ROOT.joinpath(Constants.CONTRACTS_DIR).joinpath(
        "build/mainnet.addr"
    )
    testnet_path = Constants.PROJECT_ROOT.joinpath(Constants.CONTRACTS_DIR).joinpath(
        "build/testnet.addr"
    )
    policy_id_path = Constants.PROJECT_ROOT.joinpath(Constants.CONTRACTS_DIR).joinpath(
        "build/script.policy_id"
    )
    ...
    if script == "inversionista":
        # Build script
        script_path = Constants.PROJECT_ROOT.joinpath(Constants.CONTRACTS_DIR).joinpath(
            "inversionista.py"
        )
    ...
    logging.info(
        f"Found utxo to spend: {utxo_to_spend.input.transaction_id} and index: {utxo_to_spend.input.index}"
    )
    ...
    if option == "send":
        builder.add_output(
            TransactionOutput(testnet_address, Value(20_000_00), datum=datum)
        )
    elif option == "seed":
        ...
        min_val = min_lovelace(
            ...
            output=TransactionOutput(
                testnet_address, Value(0, nft_multiasset), datum=datum
            ),
        )
        builder.add_output(
            TransactionOutput(
                testnet_address, Value(min_val, nft_multiasset), datum=datum
            )
        )
        ...
        min_val = min_lovelace(
            context,
            output=TransactionOutput(
                testnet_address, Value(0, nft_multiasset_back), datum=datum
            ),
        )
        builder.add_output(
            TransactionOutput(payment_address, Value(min_val, nft_multiasset_buy))
        )
        builder.add_output(
            TransactionOutput(
                testnet_address, Value(min_val, nft_multiasset_back), datum=datum
            )
        )
    ...
    logging.info(
        f"Cardanoscan: https://preview.cardanoscan.io/transaction/{signed_tx.id}"
    )
