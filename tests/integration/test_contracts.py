...
def create_contract(client, headers, script_type, name, wallet_id, token_name, save_flag):
    if token_name == "":
        response = client.get(
            f"/api/v1/contracts/create-contract/{script_type}?name={name}&wallet_id={wallet_id}&save_flag={str(save_flag)}",
            headers=headers
        )
    else:
        response = client.get(
            f"/api/v1/contracts/create-contract/{script_type}?name={name}&wallet_id={wallet_id}&tokenName={token_name}&save_flag={str(save_flag)}",
            headers=headers
        )
...
def test_get_scripts(client, headers):  

    response = client.get("/api/v1/contracts/get-scripts/", headers=headers)
...
def test_get_script(client, headers, query_param, test_id):
